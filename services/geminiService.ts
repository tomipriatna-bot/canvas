...
const getImageDimensions = (file: File): Promise<{ width: number; height: number }> => {
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (event) => {
            ...
            img.onerror = (err) => reject(new Error(`Kesalahan memuat gambar: ${err}`));
        };
        reader.onerror = (err) => reject(new Error(`Kesalahan pembaca file: ${err}`));
    });
};
...
const cropToOriginalAspectRatio = (
    ...
    return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
            ...
            if (!ctx) {
                return reject(new Error('Tidak bisa mendapatkan konteks kanvas untuk memotong.'));
            }
        ...
        img.onerror = (err) => reject(new Error(`Kesalahan memuat gambar saat memotong: ${err}`));
    });
};
...
const fileToPart = async (file: File): Promise<{ inlineData: { mimeType: string; data: string; } }> => {
    ...
    if (arr.length < 2) throw new Error("URL data tidak valid");
    const mimeMatch = arr[0].match(/:(.*?);/);
    if (!mimeMatch || !mimeMatch[1]) throw new Error("Tidak dapat mengurai tipe MIME dari URL data");
...
const markImage = async (
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (event) => {
            if (!event.target?.result) {
                return reject(new Error("Gagal membaca file untuk penandaan."));
            }
            const img = new Image();
            img.src = event.target.result as string;
            img.onload = () => {
                ...
                const ctx = canvas.getContext('2d');
                if (!ctx) {
                    return reject(new Error('Tidak bisa mendapatkan konteks kanvas untuk penandaan.'));
                }
                ...
                canvas.toBlob((blob) => {
                    if (blob) {
                        ...
                        reject(new Error('Konversi kanvas ke Blob gagal saat penandaan.'));
                    }
                }, 'image/jpeg', 0.95);
            };
            img.onerror = (err) => reject(new Error(`Kesalahan memuat gambar saat penandaan: ${err}`));
        };
        reader.onerror = (err) => reject(new Error(`Kesalahan pembaca file saat penandaan: ${err}`));
    });
};
...
`;
  ...
  throw new Error("Model AI tidak mengembalikan gambar. Silakan coba lagi.");
};