  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (!allowedTypes.includes(file.type)) {
        setFileTypeError('Untuk hasil terbaik, silakan gunakan format PNG, JPG, atau JPEG.');
      } else {
        setFileTypeError(null);
      }
      onFileSelect(file);
        ...
        {imageUrl ? (
          <>
            <img 
              ref={imgRef}
              src={imageUrl} 
              alt={label || 'Pemandangan yang Diunggah'} 
            ...
            {showDebugButton && onDebugClick && (
                <button
                    ...
                    aria-label="Tampilkan tampilan debug"
                >
                    Debug
                </button>
            )}
          ...
          <div className="text-center text-zinc-500 p-4">
            <UploadIcon />
            <p>Klik untuk mengunggah atau seret & lepas</p>
          </div>
        )}
      </div>
      {fileTypeError && (
